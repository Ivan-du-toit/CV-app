<h2 class="sub_title">Entries for the {{items[0].category}} category</h2>
<ul class="items">
	<li class="thumbnail" ng-repeat="item in items" ng-mouseenter="items[$index].show = true;" ng-mouseleave="items[$index].show = false;">
		...
		<span ng-show="item.show">
			...
			<span class="relatedCategory" ng-repeat="job in item.occupations"><a href="#/occupation/{{job.id}}/{{catId}}">{{job.name}}</a></span>
		</span>
